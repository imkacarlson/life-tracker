...
function serializeNode(node, indent = 0, listIndex = null) {
  ...
  switch (node.type) {
    case 'doc':
      node.content?.forEach((child) => lines.push(serializeNode(child, indent)))
      break
    case 'paragraph': {
    ...
    case 'bulletList':
      node.content?.forEach((child) => lines.push(serializeNode(child, indent, 'bullet')))
      break
    case 'orderedList': {
      let counter = 1
      node.content?.forEach((child) => {
        lines.push(serializeNode(child, indent, counter))
        counter += 1
      })
      break
    }
    case 'taskList':
      node.content?.forEach((child) => lines.push(serializeNode(child, indent, 'task')))
      break
    case 'listItem':
    case 'taskItem': {
      const marker =
      ...
      children.forEach((child, i) => {
        if (i === 0 && child.type === 'paragraph') {
          ...
          lines.push(prefix + marker + serializeInline(child.content) + childId)
        } else {
    ...
    case 'table':
      node.content?.forEach((row, rowIdx) => {
        row.content?.forEach((cell) => {
          cell.content?.forEach((child) => lines.push(serializeNode(child, indent)))
        })
    ...
    case 'tableHeader':
      ...
      break
    case 'blockquote':
      node.content?.forEach((child) => lines.push(serializeNode(child, indent + 1)))
      break
    case 'codeBlock': {
      lines.push(prefix + '```')
    ...
    case 'horizontalRule':
      ...
      break
    default:
      if (node.content) node.content.forEach((child) => lines.push(serializeNode(child, indent)))
      break
  }
  return lines.join('\n')
}
