...
// Self-hosted webfonts inlined as base64 (api/_lib/fonts/fonts.css, generated by
// build.mjs). Injecting these instead of <link>ing the CDNs means the headless
// render doesn't block on font downloads — the slow part of cold renders. The
// app's real fonts are mirrored exactly, so the preview can't drift.
let cachedFontCss = null
function loadFontCss() {
  if (cachedFontCss !== null) return cachedFontCss
  try {
    cachedFontCss = fs.readFileSync(path.join(__dirname, 'fonts', 'fonts.css'), 'utf8')
  } catch {
    cachedFontCss = '' // fall back to whatever fonts are available rather than failing the render
  }
